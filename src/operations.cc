...
template <typename F>
bool InputOperation<F>::forward_dry_run(std::vector<Tensor<F> *> &in,
...
template <typename F>
void InputOperation<F>::backward(std::vector<Tensor<F> *> &in, std::vector<Tensor<F> *> &out,
                                 std::vector<Tensor<F> *> &in_grad,
                                 std::vector<Tensor<F> *> &out_grad) {}

template <typename F>
bool InputOperation<F>::backward_dry_run(std::vector<Tensor<F> *> &in,
                                         std::vector<Tensor<F> *> &out,
                                         std::vector<Tensor<F> *> &in_grad,
                                         std::vector<Tensor<F> *> &out_grad) {
...
template <typename F> InputOperation<F>::InputOperation(cereal::PortableBinaryInputArchive &ar) {
    ar(n_channels);
}

template <typename F> void InputOperation<F>::save(cereal::PortableBinaryOutputArchive &ar) {
    ar(n_channels);
}
...
template <typename F>
bool ConvolutionOperation<F>::backward_dry_run(vector<Tensor<F> *> &in, vector<Tensor<F> *> &out,
                                               vector<Tensor<F> *> &in_grad,
                                               vector<Tensor<F> *> &out_grad) {
    if (in_grad.size()) {
...
template <typename F>
void ConvolutionOperation<F>::backward(vector<Tensor<F> *> &in, vector<Tensor<F> *> &out,
                                       vector<Tensor<F> *> &in_grad,
                                       vector<Tensor<F> *> &out_grad) {
    if (!out_grad[0]->allocated()) {
        throw DexeException("Error in ConvolutionOperation backward, output "
                            "gradient not allocated");
    }
...
template <typename F>
void ConvolutionOperation<F>::prepare_forward(Tensor<F> &in,
                                              Tensor<F> &out) { // allocates workspace

    // reshape output tensor, calculated from paddings, strides and dimensions
    // of filters
    ...
    handle_error(cudnnGetConvolutionForwardWorkspaceSize(Handler::cudnn(), in.td, filter_bank.fd,
                                                         conv, out.td, algo, &new_workspace_size));
    ...
    if (workspace_size != new_workspace_size) {
        workspace_size = new_workspace_size;
        cout << "Allocating workspace of size: " << workspace_size <<
       endl; if (workspace) { cudaFree(workspace); workspace = nullptr;
        }
...
template <typename F>
void ConvolutionOperation<F>::prepare_backward_weights(Tensor<F> &in,
                                                       Tensor<F> &out) { // allocates workspace
    // size_t new_workspace_size = workspace_size_bwd_filter;
    size_t new_workspace_size = Handler::workspace_size();

    handle_error(cudnnGetConvolutionBackwardFilterAlgorithm(
        Handler::cudnn(), in.td, out.td, conv, filter_bank.fd,
        CUDNN_CONVOLUTION_BWD_FILTER_SPECIFY_WORKSPACE_LIMIT, new_workspace_size,
        &algo_bwd_filter));
    handle_error(cudnnGetConvolutionBackwardFilterWorkspaceSize(
        Handler::cudnn(), in.td, out.td, conv, filter_bank_grad.fd, algo_bwd_filter,
        &new_workspace_size));
    ...
    workspace_size_bwd_filter = new_workspace_size;
        ...
        handle_error( cudaMalloc( (void**)&workspace_bwd_filter,
    workspace_size_bwd_filter) );
    }*/
}

template <typename F>
void ConvolutionOperation<F>::prepare_backward(Tensor<F> &in,
                                               Tensor<F> &out) { // allocates workspace
    size_t new_workspace_size = Handler::workspace_size();
    // size_t new_workspace_size = workspace_size_bwd;

    handle_error(cudnnGetConvolutionBackwardDataAlgorithm(
        Handler::cudnn(), filter_bank.fd, out.td, conv, in.td,
        CUDNN_CONVOLUTION_BWD_DATA_SPECIFY_WORKSPACE_LIMIT, new_workspace_size, &algo_bwd));
    handle_error(cudnnGetConvolutionBackwardDataWorkspaceSize(
        Handler::cudnn(), filter_bank.fd, out.td, conv, in.td, algo_bwd, &new_workspace_size));
    ...
    workspace_size_bwd = new_workspace_size;
        ...
        handle_error( cudaMalloc( (void**)&workspace_bwd,
    workspace_size_bwd) );
    }*/
}
...
template <typename F> void ConvolutionOperation<F>::l2(F l) {
    add_cuda<F>(filter_bank.ptr(), filter_bank_grad.ptr(), filter_bank.n_weights(), -l);
}

template <typename F> void ConvolutionOperation<F>::init_normal(F mean, F std) {
    filter_bank.init_normal(mean, std);
    // bias.init_normal(mean, std);
}

template <typename F> void ConvolutionOperation<F>::init_uniform(F var) {
    filter_bank.init_uniform(var);
    // bias.init_uniform(var);
}

template <typename F> vector<F> ConvolutionOperation<F>::to_vector() {
...
template <typename F> void ConvolutionOperation<F>::from_vector(vector<F> &v) {
    ...
    if (has_bias) {
        vector<F> bias_weights(v.begin() + filter_bank.n_weights(),
                               v.begin() + filter_bank.n_weights() + bias.size());
        bias.from_vector(bias_weights);
    }
}

template <typename F> int ConvolutionOperation<F>::size() {
    return filter_bank.n_weights() + (has_bias ? bias.size() : 0);
}

template <typename F> vector<F> ConvolutionOperation<F>::grad_to_vector() {
...
void ConvolutionOperation<F>::forward(Tensor<F> &input, Tensor<F> &output, F beta) {
    ...
    // cout << dimensions << " " << strides << " " << paddings << " " <<
    // dilations << endl; cout << filter_bank << " " << filter_bank.weights <<
    // endl;
    // cout << workspace << " " << workspace_size << endl;
    handle_error(cudnnConvolutionForward(Handler::cudnn(), &alpha, input.td, input.ptr(),
                                         filter_bank.fd, filter_bank.weights.ptr(), conv, algo, workspace,
                                         workspace_size, &beta, output.td, output.ptr()));
    // handle_error( cudnnAddTensor(Handler::cudnn(), CUDNN_ADD_FEATURE_MAP,
    // &alpha_bias, bias.td, bias.data, &beta_bias, output.td, output.data));
    // auto v = bias.to_vector();
    if (has_bias)
        handle_error(cudnnAddTensor(Handler::cudnn(), &alpha_bias, bias.td, bias.ptr(), &beta_bias,
                                    output.td, output.ptr()));
}

template <typename F>
void ConvolutionOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &input_grad,
                                       Tensor<F> &output_grad, F beta) {
    F alpha(1.0);
    // cout << ":in out filter: " << input_grad.shape << " " <<
    // output_grad.shape << " " << filter_bank << endl; cout << strides << " "
    // << dimensions << " " << paddings << endl; algo_bwd =
    // CUDNN_CONVOLUTION_BWD_DATA_ALGO_0;
    handle_error( cudnnConvolutionBackwardData(Handler::cudnn(), &alpha, filter_bank.fd,
                                              filter_bank.ptr(), output_grad.td, output_grad.ptr(),
                                              conv, algo_bwd, workspace_bwd, workspace_size_bwd,
                                              &beta, input_grad.td, input_grad.ptr()) );
}
...
void ConvolutionOperation<F>::backward_weights(Tensor<F> &input, Tensor<F> &output_grad, F beta) {
    if (has_bias) {
        ...
        handle_error( cudnnConvolutionBackwardBias(Handler::cudnn(), &alpha_bias, output_grad.td,
                                                  output_grad.ptr(), &beta_bias, bias_grad.td,
                                                  bias_grad.ptr()) );
    }

    F alpha(1.0);
    handle_error(cudnnConvolutionBackwardFilter(
        Handler::cudnn(), &alpha, input.td, input.ptr(), output_grad.td, output_grad.ptr(), conv,
        algo_bwd_filter, workspace_bwd_filter, workspace_size_bwd_filter, &beta,
        filter_bank_grad.fd, filter_bank_grad.ptr()));
}

template <typename F> void ConvolutionOperation<F>::zero_grad() {
    filter_bank_grad.zero();
    if (has_bias)
        bias_grad.zero();
}

template <typename F> TensorShape ConvolutionOperation<F>::output_shape(TensorShape in) {
    in.set_c(filter_bank.out_c());
    return in;
}

template <typename F> void ConvolutionOperation<F>::scale_grad(F val) {
...
template <typename F> void ConvolutionOperation<F>::share(ConvolutionOperation<F> &other) {
    other.filter_bank.weights.share(filter_bank.weights);
    other.filter_bank_grad.weights.share(filter_bank_grad.weights);

    if (has_bias) {
        bias.share(other.bias);
        bias_grad.share(other.bias_grad);
    }
}
...
void ConvolutionTransposeOperation<F>::forward(std::vector<Tensor<F> *> &in,
                                               std::vector<Tensor<F> *> &out) {
    Tensor<F> dummy;
    ConvolutionOperation<F>::backward(
        dummy, dummy, *out[0],
        *in[0]); // we use ConvolutionOperation in reverse to get the transpose
}

template <typename F>
bool ConvolutionTransposeOperation<F>::forward_dry_run(std::vector<Tensor<F> *> &in,
                                                       std::vector<Tensor<F> *> &out) {
    auto in_shape = in[0]->shape;
    // In the transpose we use cudnnConvolutions in reverse, so the output
    // dimension is not the 1st dimension, input the 0th.
...
template <typename F>
void ConvolutionTransposeOperation<F>::backward(std::vector<Tensor<F> *> &in,
                                                std::vector<Tensor<F> *> &out,
                                                std::vector<Tensor<F> *> &in_grad,
                                                std::vector<Tensor<F> *> &out_grad) {
    if (in_grad.size())
        ConvolutionOperation<F>::forward(*out_grad[0],
                                         *in_grad[0]); // we use ConvolutionOperation in
                                                       // reverse to get the transpose
    ConvolutionOperation<F>::backward_weights(
        *out_grad[0],
        *in[0]); // we use ConvolutionOperation in reverse to get the transpose
}

template <typename F>
bool ConvolutionTransposeOperation<F>::backward_dry_run(std::vector<Tensor<F> *> &in,
                                                        std::vector<Tensor<F> *> &out,
                                                        std::vector<Tensor<F> *> &in_grad,
                                                        std::vector<Tensor<F> *> &out_grad) {
    if (in_grad.size())
...
template <typename F>
ConvolutionTransposeOperation<F>::ConvolutionTransposeOperation(
    cereal::PortableBinaryInputArchive &ar)
    : ConvolutionOperation<F>(ar) {}
...
/////////////////////
template <typename F> SquaredLossOperation<F>::SquaredLossOperation() {}
...
template <typename F>
bool SquaredLossOperation<F>::backward_dry_run(std::vector<Tensor<F> *> &in,
                                               std::vector<Tensor<F> *> &out,
                                               std::vector<Tensor<F> *> &in_grad,
                                               std::vector<Tensor<F> *> &out_grad) {
    in_grad[0]->reshape(in[0]->shape);
...
void SupportLossOperation<F>::forward(std::vector<Tensor<F> *> &in, std::vector<Tensor<F> *> &out) {
    support_loss(in[0]->ptr(), in[1]->ptr(), tmp.ptr(), in[0]->shape.n_elements(), support);
...
template <typename F>
bool SupportLossOperation<F>::forward_dry_run(std::vector<Tensor<F> *> &in,
                                              std::vector<Tensor<F> *> &out) {
    if (in.size() != 2) {
...
template <typename F>
bool SupportLossOperation<F>::backward_dry_run(std::vector<Tensor<F> *> &in,
                                               std::vector<Tensor<F> *> &out,
                                               std::vector<Tensor<F> *> &in_grad,
                                               std::vector<Tensor<F> *> &out_grad) {
    in_grad[0]->reshape(in[0]->shape);
...
template <typename F>
void SupportLossOperation<F>::backward(std::vector<Tensor<F> *> &in, std::vector<Tensor<F> *> &out,
                                       std::vector<Tensor<F> *> &in_grad,
                                       std::vector<Tensor<F> *> &out_grad) {
    in_grad[0]->from_tensor(tmp, 1.0 / tmp.shape.n_elements());
}

template <typename F> void SupportLossOperation<F>::save(cereal::PortableBinaryOutputArchive &ar) {
    ar(support);
}
...
template <typename F> void SquashOperation<F>::init_normal(F mean, F std) {
    this->filter_bank.init_normal(
        mean, std / (this->filter_bank.kd() * this->filter_bank.kw() * this->filter_bank.kh()));
    // bias.init_normal(mean, std);
}

template <typename F> void SquashOperation<F>::init_uniform(F var) {
...
template <typename F> UnsquashOperation<F>::UnsquashOperation(TensorShape s_) : s(s_) {}

template <typename F> TensorShape UnsquashOperation<F>::output_shape(TensorShape in) { return s; }

template <typename F> void UnsquashOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    out.from_tensor(in);
}

template <typename F>
void UnsquashOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &in_grad,
...
template <typename F> MergeOperation<F>::MergeOperation() {}

template <typename F> TensorShape MergeOperation<F>::output_shape(TensorShape in) {
    return TensorShape{in.n(), in.c() / 4, in.w() * 2, in.h() * 2};
}

template <typename F> void MergeOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    merge(in, out);
}

template <typename F>
void MergeOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &in_grad,
                                 Tensor<F> &out_grad, F beta) {
...
template <typename F> SplitOperation<F>::SplitOperation() {}

template <typename F> TensorShape SplitOperation<F>::output_shape(TensorShape in) {
    return TensorShape{in.n(), in.c() * 4, in.w() / 2, in.h() / 2};
}

template <typename F> void SplitOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    split(in, out);
}

template <typename F>
void SplitOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &in_grad,
                                 Tensor<F> &out_grad, F beta) {
...
template <typename F> LocalNormalisationOperation<F>::LocalNormalisationOperation(int w_) : w(w_) {
    handle_error(cudnnCreateLRNDescriptor(&lrn_desc));
    // handle_error( cudnnSetLRNDescriptor( lrn_desc, w, 1.0, 1.0e-4, 0.75) );
}

template <typename F> TensorShape LocalNormalisationOperation<F>::output_shape(TensorShape input) {
    return input;
}
...
void LocalNormalisationOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    F alpha(1.0);
    handle_error(cudnnLRNCrossChannelForward(Handler::cudnn(), lrn_desc,
                                             CUDNN_LRN_CROSS_CHANNEL_DIM1, &alpha, in.td, in.ptr(),
                                             &beta, out.td, out.ptr()));
}

template <typename F>
void LocalNormalisationOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &in_grad,
                                              Tensor<F> &out_grad, F beta) {
    F alpha(1.0);
    handle_error(cudnnLRNCrossChannelBackward(
        Handler::cudnn(), lrn_desc, CUDNN_LRN_CROSS_CHANNEL_DIM1, &alpha, out.td, out.ptr(),
        out_grad.td, out_grad.ptr(), in.td, in.ptr(), &beta, in_grad.td, in_grad.ptr()));
}

template <typename F>
LocalNormalisationOperation<F>::LocalNormalisationOperation(
    cereal::PortableBinaryInputArchive &ar) {
    ar(w);
    handle_error(cudnnCreateLRNDescriptor(&lrn_desc));
}
...
template <typename F>
PoolingOperation<F>::PoolingOperation(int kw_, int kh_, cudnnPoolingMode_t mode)
    : kw(kw_), kh(kh_) {
...
template <typename F> void PoolingOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    F alpha(1.0);
    handle_error(cudnnPoolingForward(Handler::cudnn(), pool, &alpha, in.td, in.ptr(), &beta, out.td,
                                     out.ptr()));
}
...
template <typename F> TanhOperation<F>::TanhOperation(F scale_) : scale(scale_) {
    cudnnCreateActivationDescriptor(&desc);
    cudnnSetActivationDescriptor(desc, CUDNN_ACTIVATION_TANH, CUDNN_NOT_PROPAGATE_NAN, 0);
}

template <typename F> void TanhOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    F alpha(1);
...
template <typename F>
void TanhOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &in_grad,
                                Tensor<F> &out_grad, F beta) {
    F alpha(1);
...
template <typename F> TensorShape TanhOperation<F>::output_shape(TensorShape in) { return in; }

template <typename F> AdditionOperation<F>::AdditionOperation() {}
...
template <typename F>
void AdditionOperation<F>::backward(std::vector<Tensor<F> *> &in, std::vector<Tensor<F> *> &out,
                                    std::vector<Tensor<F> *> &in_grad,
                                    std::vector<Tensor<F> *> &out_grad) {
...
template <typename F>
bool AdditionOperation<F>::backward_dry_run(std::vector<Tensor<F> *> &in,
                                            std::vector<Tensor<F> *> &out,
                                            std::vector<Tensor<F> *> &in_grad,
                                            std::vector<Tensor<F> *> &out_grad) {
    in_grad[0]->reshape(in[0]->shape);
...
template <typename F> TensorShape AdditionOperation<F>::output_shape(TensorShape in) { return in; }

template <typename F> SigmoidOperation<F>::SigmoidOperation(F scale_) : scale(scale_) {
    cudnnCreateActivationDescriptor(&desc);
    cudnnSetActivationDescriptor(desc, CUDNN_ACTIVATION_SIGMOID, CUDNN_NOT_PROPAGATE_NAN, 0);
}

template <typename F> void SigmoidOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    F alpha(1);
    sigm_forward<F>(in.ptr(), out.ptr(), out.size(), beta, scale);
}

template <typename F>
void SigmoidOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &in_grad,
                                   Tensor<F> &out_grad, F beta) {
    sigm_deriv<F>(out_grad.ptr(), out.ptr(), in_grad.ptr(), out.size(), beta, scale);
}
...
template <typename F> void ReluOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    F alpha(1);
    handle_error(cudnnActivationForward(Handler::cudnn(), desc, &alpha, in.td, in.ptr(), &beta,
                                        out.td, out.ptr()));
}
...
template <typename F>
void ReluOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &in_grad,
                                Tensor<F> &out_grad, F beta) {
    F alpha(1);
...
template <typename F> void SoftmaxOperation<F>::forward(Tensor<F> &in, Tensor<F> &out, F beta) {
    F alpha(1);
    handle_error(cudnnSoftmaxForward(Handler::cudnn(), CUDNN_SOFTMAX_ACCURATE,
                                     CUDNN_SOFTMAX_MODE_CHANNEL, &alpha, in.td, in.ptr(), &beta,
                                     out.td, out.ptr()));
}

template <typename F>
void SoftmaxOperation<F>::backward(Tensor<F> &in, Tensor<F> &out, Tensor<F> &in_grad,
                                   Tensor<F> &out_grad, F beta) {
    F alpha(1);
    ...
    if (matched) { // loss function matched
        ...
        handle_error(cudnnSoftmaxBackward(
            Handler::cudnn(), CUDNN_SOFTMAX_ACCURATE, CUDNN_SOFTMAX_MODE_CHANNEL, &alpha, out.td,
            out.ptr(), out_grad.td, out_grad.ptr(), &beta, in_grad.td, in_grad.ptr()));
}

template <typename F> TensorShape SoftmaxOperation<F>::output_shape(TensorShape in) { return in; }

//////// Instance Norm
template <typename F> InstanceNormalisationOperation<F>::InstanceNormalisationOperation() {}

// Runs the forward step
template <typename F>
void InstanceNormalisationOperation<F>::forward(std::vector<Tensor<F> *> &in,
                                                std::vector<Tensor<F> *> &out) {
    auto &in_tensor(*in[0]);
    auto &out_tensor(*out[0]);
...
// Responsible for both checking if sizes match, and making sure the memory is
// allocated
template <typename F>
bool InstanceNormalisationOperation<F>::forward_dry_run(std::vector<Tensor<F> *> &in,
                                                        std::vector<Tensor<F> *> &out) {
...
template <typename F>
void InstanceNormalisationOperation<F>::backward(std::vector<Tensor<F> *> &in,
                                                 std::vector<Tensor<F> *> &out,
                                                 std::vector<Tensor<F> *> &in_grad,
                                                 std::vector<Tensor<F> *> &out_grad) {
...
template <typename F>
bool InstanceNormalisationOperation<F>::backward_dry_run(std::vector<Tensor<F> *> &in,
                                                         std::vector<Tensor<F> *> &out,
                                                         std::vector<Tensor<F> *> &in_grad,
                                                         std::vector<Tensor<F> *> &out_grad) {
    in_grad[0]->reshape(in[0]->shape);
...
template <typename F> void InstanceNormalisationOperation<F>::describe(std::ostream &out) {
    out << "instance_norm";
}
