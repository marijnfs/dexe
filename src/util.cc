...
template <>
void init_normal(float *data, int n, float mean, float std) {
	assert(n > 1);
	handle_error( curandGenerateUniform ( Handler::curand(), data, n) );
	range<float>(data, n, -std, std);
	// handle_error( curandGenerateNormal ( Handler::curand(), data, (n / 2 * 2), mean, std) );
	// if (n % 2)
...
template <>
void init_normal(double *data, int n, double mean, double std) {
	assert(n > 1);
