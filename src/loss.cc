...
template <typename F> F Loss<F>::loss() { return this->last_loss; }

template <typename F> int Loss<F>::n_correct() { return last_correct; }

template <typename F>
SoftmaxLoss<F>::SoftmaxLoss(int n_, int c_) : Loss<F>(n_, c_) {}

template <typename F>
void SoftmaxLoss<F>::calculate_loss(Tensor<F> &in, vector<int> answers,
                                    Tensor<F> &err) {
    this->last_loss = 0;
    this->last_correct = 0;
    const F e(.00000001);
...
template <typename F>
void SoftmaxLoss<F>::calculate_loss(Tensor<F> &in, Tensor<F> &target,
                                    Tensor<F> &err) {
    assert(in.size() == target.size());
    this->last_loss = 0;
    // Loss<F>::last_loss = 0;
    this->last_correct = 0;
...
template <typename F>
void SquaredLoss<F>::calculate_loss(Tensor<F> &in, vector<int> answers,
                                    Tensor<F> &err) {
    this->last_loss = 0;
    this->last_correct = 0;

    assert(in.size() == err.size());
