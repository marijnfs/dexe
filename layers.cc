...
ConvolutionLayer::ConvolutionLayer(int in_map_, int out_map_, int kw_, int kh_, Tensor &input):
	in_map(in_map_), out_map(out_map_), kw(kw_), kh(kh_)
{
	handle_error( cudnnCreateFilterDescriptor(&filter));
	handle_error( cudnnSetFilter4dDescriptor(filter, CUDNN_DATA_FLOAT, out_map, in_map, kh, kw));
...
void ConvolutionLayer::init(Tensor &input) {
...
void BiasLayer::forward(Tensor &bias, Tensor &out) {
	float alpha(1), beta(1);
	handle_error( cudnnAddTensor(Handler::cudnn(), CUDNN_ADD_SAME_C, &alpha, bias.td, bias.data, &beta, out.td, out.data));
}
